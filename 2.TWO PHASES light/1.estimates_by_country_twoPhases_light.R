...
# s2022=s2022[s2022$SURVEY_LC1 != "8", ]
# s2022=s2022[s2022$SURVEY_LU1 != "8", ]
# write.table(s2022,file=paste0(path_data,"Survey_2022_cal_wgt_standard.txt"),quote=F,sep="\t",row.names = F)
...
for (i in c(1:length(paesi))) {
  ...
  # 2nd PHASE
  #---------------------------------------------------------------
  source("Script/1.estimates_by_country_twoPhases_secondPhase_simplified_light.R")
