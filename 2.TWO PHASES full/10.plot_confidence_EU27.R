...
for (i in (1:nrow(eu1))) {
  ...
  df <- NULL
  df$Year <- years
  df$var <- c(eu1$Area2015[i],eu1$Area2018[i],eu1$Area2022[i])
  df$ci_l <- c(eu1$CI.l_2015[i],eu1$CI.l_2018[i],eu1$CI.l_2022[i])
  df$ci_u <- c(eu1$CI.u_2015[i],eu1$CI.u_2018[i],eu1$CI.u_2022[i])
  df <- as.data.frame(df)
  colnames(df)[2] <- var
  df
  plotCI(df=df,
