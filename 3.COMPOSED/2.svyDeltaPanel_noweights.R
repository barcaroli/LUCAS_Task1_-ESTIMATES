...
des_2018 <- e.svydesign(data=s2018, ids= ~ POINT_ID, strata= ~ stratum,
                        weights= ~ ones, self.rep.str= NULL, check.data= TRUE)
ls <- find.lon.strata(des_2018)
if (!is.null(ls)) des_2018 <- collapse.strata(des_2018)
# save(des_2018,file="des_2018.RData")

sum(s2018$ones)
sum(s2018$ones*s2018$LC2018A)/sum(s2018$ones)
sum(s2018$ones*s2018$LC2018B)/sum(s2018$ones)
sum(s2018$ones*s2018$LC2018C)/sum(s2018$ones)
sum(s2018$ones*s2018$LC2018D)/sum(s2018$ones)
sum(s2018$ones*s2018$LC2018E)/sum(s2018$ones)
sum(s2018$ones*s2018$LC2018F)/sum(s2018$ones)
sum(s2018$ones*s2018$LC2018G)/sum(s2018$ones)
sum(s2018$ones*s2018$LC2018H)/sum(s2018$ones)
# 
# 
# #-----------
# # LUCAS 2022
...
sum(s2022$ones)
sum(s2022$ones*s2022$LC2022A)/sum(s2022$ones)
sum(s2022$ones*s2022$LC2022B)/sum(s2022$ones)
sum(s2022$ones*s2022$LC2022C)/sum(s2022$ones)
sum(s2022$ones*s2022$LC2022D)/sum(s2022$ones)
sum(s2022$ones*s2022$LC2022E)/sum(s2022$ones)
sum(s2022$ones*s2022$LC2022F)/sum(s2022$ones)
sum(s2022$ones*s2022$LC2022G)/sum(s2022$ones)
sum(s2022$ones*s2022$LC2022H)/sum(s2022$ones)
# load("svyDeltaPanel.RData")
#-------------------------------
# Calculate 2018/2022 variations
#-------------------------------

(sum(s2022$ones*s2022$LC2022C)/sum(s2022$ones) - sum(s2018$ones*s2018$LC2018C)/sum(s2018$ones)) / 
  (sum(s2022$ones*s2022$LC2022C)/sum(s2022$ones))
...
artificial <- svyDelta(expr=expression((LC2022A.2 - LC2018A.1)/LC2018A.1),
                       design1=des_2018, design2=des_2022, by = NULL,
...
artificial_NUTS0 <- svyDelta(expr=expression((LC2022A.2 - LC2018A.1)/LC2018A.1),
                             design1=des_2018, design2=des_2022, by = ~ NUTS0_16,
                             des.INDEP = FALSE, rho.STRAT = c("noStrat"),
                             vartype = c("se", "cv", "cvpct"),
                             conf.int = TRUE, conf.lev = 0.95)
artificial_NUTS0
