...
survey2022 <- read.delim("Survey_2022_cal_wgt.txt")
s2022 <- NULL
...
des <- e.svydesign(data=europa22, ids= ~ ID, strata= ~ stratum, weights= ~ wgts, 
...
save(des,file="Europe22_1digit.RData")

est_eu22 <- svystatTM(des, ~ area +
                        land_cover_1 +
                        land_use_1 ,
                      # settlement +
                      # settl_pc +
...
des <- e.svydesign(data=europa26, ids= ~ ID, strata= ~ stratum, weights= ~ wgts, 
                   self.rep.str= NULL, check.data= TRUE)
...
write.xlsx(est_eu26_t, file=paste0(path_output,"Europe_LC_LU_1digit.xlsx"), sheetName = "Europe26", 
           col.names = FALSE, row.names = FALSE, append = TRUE)
# write.table(est_eu26,".\\EU_estimates\\Europe26.csv",sep = ";",dec=".",row.names=F)
...
write.xlsx(est_eu27_t, file=paste0(path_output,"Europe_LC_LU_1digit.xlsx"), sheetName = "Europe27", 
           col.names = FALSE, row.names = FALSE, append = TRUE)

# write.table(est_eu27,".\\EU_estimates\\Europe27_onedigit.csv",sep = ";",dec=".",row.names=F)
