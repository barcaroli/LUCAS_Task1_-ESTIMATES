...
for(i in countries){
  ...
  Y1r <- svystat(cal, y = ~LC+LU, by = ~NUTS2_24, estimator="Total", conf.int=TRUE)
  Y2r <- svystat(cal, y = ~LC+LU, by = ~NUTS2_24, estimator="Mean", conf.int=TRUE)
  if (nrow(Y2r) < 12) {
    nuts <- levels(as.factor(s2022$NUTS2_24))
  ...
  if (length(a) == 8 & (length(b) == 4)) {
    y1s <- svystat(cal_new, y = ~LC2022+LU2022, by = ~NUTS2_24, estimator="Total", conf.int=TRUE)
    y2s <- svystat(cal_new, y = ~LC2022+LU2022, by = ~NUTS2_24, estimator="Mean", conf.int=TRUE)
  ...
  est <- rbind(est,estim)
  print(est)
  filename <- paste(path_out,i,'_est_LC1_LU1_NUTS2_24_2022.csv',sep="")
