#---------------------------------------------------------------------------------
# Script to estimate at EU level (22, 26, 26) - Land cover and Land use 2 digits
#---------------------------------------------------------------------------------
options(stringsAsFactors = TRUE)
library(xlsx)
library(data.table)
path_output <- "D:/Google Drive/LUCAS 2025/Task 1 - ESTIMATES/0.CURRENT/EU_estimates/"
...
europa22$land_cover_1 <- as.factor(substr(europa22$land_cover,1,2))
europa22$land_use_1 <- as.factor(substr(europa22$land_use,1,3))
...
write.xlsx(est_eu22_t, file=paste0(path_output,"Europe_LC_LU_2digit.xlsx"), 
           sheetName = "Europe22", 
...
europa26$land_cover_1 <- as.factor(substr(europa26$land_cover,1,2))
europa26$land_use_1 <- as.factor(substr(europa26$land_use,1,3))
...
write.xlsx(est_eu26_t, file=paste0(path_output,"Europe_LC_LU_2digit.xlsx"), 
           sheetName = "Europe26", 
...
europa27$land_cover_1 <- as.factor(substr(europa27$land_cover,1,2))
europa27$land_use_1 <- as.factor(substr(europa27$land_use,1,3))
...
write.xlsx(est_eu27_t, file=paste0(path_output,"Europe_LC_LU_2digit.xlsx"), 
           sheetName = "Europe27", 
           col.names = FALSE, row.names = FALSE, append = TRUE)
save.image(file=paste0(path_output,"Europe_2digit.RData"))