...
load(paste0(data_path,"master_total.RData"))
master_total$LU_pred <- as.factor(master_total$LU)
table(master_total$LC_pred)
table(master_total$LU_pred)
...
s2022$LC_pred <- s2022$LC_pred.x <- s2022$LC_pred.y <- s2022$LU_pred <- s2022$LU_pred.x <- s2022$LU_pred.y <- NULL


# #-----------
# # LUCAS 2018
# #-----------
s2018 <- merge(s2018,master_total[,c("POINT_ID","LC_pred","LU_pred"),],by="POINT_ID")
table(s2018$LC_pred,useNA="ifany")
table(s2018$LU_pred,useNA="ifany")
sum(s2018$cal_wgt)
...
poptot2018 <- pop.template(data=des_2018,
                           calmodel= ~ LC_pred + LU_pred - 1, partition= FALSE)
poptot2018 <- fill.template(universe=master_total,template=poptot2018)
...
# # LUCAS 2022
# #-----------
s2022 <- merge(s2022,master_total[,c("POINT_ID","LC_pred","LU_pred"),],by="POINT_ID")
table(s2022$LC_pred,useNA="ifany")
table(s2022$LU_pred,useNA="ifany")
sum(s2018$cal_wgt)
